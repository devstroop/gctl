...
pub fn main() !void {
    ...
    try providers.execute(allocator, &stdout, &stderr, ctxs, token, result.command, result.number, result.provider_url, result.path, result.name, result.description, result.private, result.labels, result.title, result.base, result.quick, result.all, result.source, result.target);
}
