...
pub fn main() !void {
    ...
    try providers.execute(allocator, &stdout, &stderr, ctx, token, result.command, result.number, result.provider_url, result.name, result.description, result.private);
}
