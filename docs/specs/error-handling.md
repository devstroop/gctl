...
|---|---|
| No git repo | `Not a git repository. Run gctl inside a git repo.` |
| No remote | `No git remote found. Add a remote with 'git remote add'.` |
| Unknown provider | `Could not detect provider from remotes. Run 'gctl context'.` |
| No token | `No token for {provider}. Set {ENV}_TOKEN or run 'gctl auth login'.` |
| 401 Unauthorized | `Authentication failed. Check your token or run 'gctl auth login'.` |
...
Error propagation uses Zig's explicit error union types throughout. All `gctl` errors are either:
